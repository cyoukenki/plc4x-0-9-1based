...
public abstract class GeneratedDriverByteToMessageCodec<T extends Message> extends ByteToMessageCodec<T> {
    ...
    protected void encode(ChannelHandlerContext ctx, T packet, ByteBuf byteBuf) {
        LOGGER.debug("GeneratedDriverByteToMessageCodec::message from driver byte to message codec");
        try {
            ...
            LOGGER.debug("encode and writeBytes content [{}]",
                Hex.encodeHexString(buffer.getData()));
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf byteBuf, List<Object> out) {
        LOGGER.debug("Receiving bytes, trying to decode Message...byteBuf readableBytes ========"+byteBuf.readableBytes());
        ...
        while (byteBuf.readableBytes() > 0) {
            ...
            try {
                ...
                LOGGER.debug("decoding package with content [{}]",
                ...
                LOGGER.warn("decoding package with exception",e);
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        LOGGER.debug("Client " + ctx.channel().remoteAddress().toString() + " is connected");
        super.channelActive(ctx);
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        LOGGER.debug("Client " + ctx.channel().remoteAddress().toString() + " is disconnected");
        super.channelInactive(ctx);
    }

    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        LOGGER.error("GeneratedDriverByteToMessageCodec: Exception caught with client " + ctx.channel().remoteAddress().toString(), cause);
