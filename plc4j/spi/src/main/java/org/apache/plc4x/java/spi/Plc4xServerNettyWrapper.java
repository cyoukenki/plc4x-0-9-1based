...
public class Plc4xServerNettyWrapper<T> extends MessageToMessageCodec<T, Object> {

    private static final Logger logger = LoggerFactory.getLogger(Plc4xServerNettyWrapper.class);
    ...
    public Plc4xServerNettyWrapper(ChannelPipeline pipeline, boolean passive, Plc4xProtocolBase<T> protocol, Class<T> clazz, DefaultServerConnectorListener defaultServerConnectorListener) {
        super(clazz, Object.class);
        this.pipeline = pipeline;
        this.passive = passive;
        this.registeredHandlers = new ConcurrentLinkedQueue<>();
        this.defaultServerConnectorListener = defaultServerConnectorListener;
        ...
        this.protocolBase.setContext(new ConversationContext<T>() {
            ...
            @Override
            public DefaultServerConnectorListener getDefaultServerConnectorListener() {
                return defaultServerConnectorListener;
            }
        });
    }
...
//        }, plcRequestContainer);
    ...
    protected void decode(ChannelHandlerContext channelHandlerContext, T t, List<Object> list) throws Exception {
        ...
        try {
            ...
            protocolBase.decode(new DefaultConversationContext<>(channelHandlerContext, passive,defaultServerConnectorListener), t);
        } catch (Exception e) {
            logger.error("Plc4xServerNettyWrapper decode with error",e);
        }
    }
    ...
    public class DefaultConversationContext<T1> implements ConversationContext<T1> {
        ...
        public DefaultConversationContext(ChannelHandlerContext channelHandlerContext, boolean passive, DefaultServerConnectorListener defaultServerConnectorListener) {
            this.channelHandlerContext = channelHandlerContext;
            this.passive = passive;
            this.defaultServerConnectorListener = defaultServerConnectorListener;
    ...
    public void channelRegistered(ChannelHandlerContext ctx) throws Exception {
        logger.debug("Plc4xServerNettyWrapper: Client " + ctx.channel().remoteAddress().toString() + " channelRegistered");
        super.channelRegistered(ctx);
    }

    @Override
    public void channelUnregistered(ChannelHandlerContext ctx) throws Exception {
        logger.debug("Plc4xServerNettyWrapper: Client " + ctx.channel().remoteAddress().toString() + " channelUnregistered");
        super.channelUnregistered(ctx);
    }

    @Override
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        logger.debug("Plc4xServerNettyWrapper: Client " + ctx.channel().remoteAddress().toString() + " is connected");
        this.protocolBase.onHostConnect(new DefaultConversationContext<>(ctx, passive,defaultServerConnectorListener));
        super.channelActive(ctx);
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        logger.debug("Plc4xServerNettyWrapper: Client " + ctx.channel().remoteAddress().toString() + " is disconnected");
        this.protocolBase.onHostDisconnect(new DefaultConversationContext<>(ctx, passive));
        super.channelInactive(ctx);
    }

    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        logger.error("Plc4xServerNettyWrapper: Exception caught with client " + ctx.channel().remoteAddress().toString(), cause);
