...
public class CachedDriverManager extends PlcDriverManager implements CachedDriverManagerMBean {
    ...
    public void returnConnection(PlcConnection activeConnection) {
        synchronized (this) {
    ...
    public PlcConnection getConnection(String url) throws PlcConnectionException {
        ...
        synchronized (this) {
            String cachedUrl = this.url.replaceFirst("plc4x:/?/?", "").split("\\?")[0];
            String connectionUrl = url.replaceFirst("plc4x:/?/?", "").split("\\?")[0];
            if (!cachedUrl.equals(connectionUrl)) {
                throw new IllegalArgumentException("This Cached Driver Manager only supports the Connection " + url);
            }

            if (this.activeConnection == null) {
                setState(ConnectionState.DISCONNECTED);
            } else if (this.activeConnection != null && !this.activeConnection.isConnected()) {
                logger.info("CachedDriverManager--getConnection--state:{}--queue size is {}--activeConnection", state.get(), queue.size());
                try {
                    this.activeConnection.close();
                } catch (Exception ignore) {
                }
            ...
            if (queue.isEmpty() && isConnectionAvailable()) {
                ...
                try {
                    ...
                    logger.warn("Getting connection fail", ignore);
                }
            }
