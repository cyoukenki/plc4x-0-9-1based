...
public class TriggeredScraperTask implements ScraperTask, TriggeredScraperTaskMBean {
    ...
    @Override
    public void run() {
        LOGGER.debug("TriggeredScraperTask--task running ");
        ...
        if(this.triggerHandler.checkTrigger()) {
            ...
            // LOGGER.info("d-d-d-d-d-d-d-d-d-d--d-d-d-d:{}","before try get connection");
            try {
                String info = "";
                if(LOGGER.isTraceEnabled()) {
                ...
                // LOGGER.info("d-d-d-d-d-d-d-d-d-d--d-d-d-d:{}","before get connection");
                ...
                try {
                    ...
                    LOGGER.debug("TriggeredScrapTask readRequest error",e);
                    handleException(e);
                    return;
                }
                ...
                if (connection != null) {
                     LOGGER.debug("TriggeredScrapTask connection close----"+connection.getClass().getName());
                    try {
                        connection.close();
                    } catch (Exception e) {
