...
public class VariantStatusCodeIO implements MessageIO<VariantStatusCode, VariantStatusCode> {

    private static final Logger LOGGER = LoggerFactory.getLogger(VariantStatusCodeIO.class);

    @Override
    public VariantStatusCode parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (VariantStatusCode) new VariantIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, VariantStatusCode value, Object... args) throws ParseException {
        new VariantIO().serialize(writeBuffer, value, args);
    }

    public static VariantStatusCodeBuilder staticParse(ReadBuffer readBuffer, Boolean arrayLengthSpecified) throws ParseException {
        ...
        StatusCode[] value;
        {
            ...
            value = new StatusCode[itemCount];
            for(int curItem = 0; curItem < itemCount; curItem++) {
                boolean lastItem = curItem == (itemCount - 1);
value[curItem] = StatusCodeIO.staticParse(readBuffer ) ;            }
        }
            readBuffer.closeContext("value", WithReaderWriterArgs.WithRenderAsList(true));

        readBuffer.closeContext("VariantStatusCode");
        // Create the instance
        return new VariantStatusCodeBuilder(arrayLength, value);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, VariantStatusCode _value) throws ParseException {
        ...
        if(_value.getValue() != null) {
            ...
            int curItem = 0;
            for(StatusCode element : _value.getValue()) {
                boolean lastItem = curItem == (itemCount - 1);
    ...
    public static class VariantStatusCodeBuilder implements VariantIO.VariantBuilder {
        private final Integer arrayLength;
        private final StatusCode[] value;

        public VariantStatusCodeBuilder(Integer arrayLength, StatusCode[] value) {
            this.arrayLength = arrayLength;
            this.value = value;
        }

        public VariantStatusCode build(boolean arrayLengthSpecified, boolean arrayDimensionsSpecified, Integer noOfArrayDimensions, boolean[] arrayDimensions) {
