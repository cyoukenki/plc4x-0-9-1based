...
public class ReadResponseIO implements MessageIO<ReadResponse, ReadResponse> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReadResponseIO.class);

    @Override
    public ReadResponse parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (ReadResponse) new ExtensionObjectDefinitionIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, ReadResponse value, Object... args) throws ParseException {
        new ExtensionObjectDefinitionIO().serialize(writeBuffer, value, args);
    }

    public static ReadResponseBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
...
int noOfResults = readBuffer.readInt("noOfResults", 32) ;        // Array field (results)
        ...
        DataValue[] results;
        {
            ...
            results = new DataValue[itemCount];
            for(int curItem = 0; curItem < itemCount; curItem++) {
                boolean lastItem = curItem == (itemCount - 1);
results[curItem] = DataValueIO.staticParse(readBuffer ) ;            }
        }
...
diagnosticInfos[curItem] = DiagnosticInfoIO.staticParse(readBuffer ) ;            }
        ...
        // Create the instance
        return new ReadResponseBuilder(responseHeader, noOfResults, results, noOfDiagnosticInfos, diagnosticInfos);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, ReadResponse _value) throws ParseException {
        ...
        if(_value.getResults() != null) {
            ...
            int curItem = 0;
            for(DataValue element : _value.getResults()) {
                boolean lastItem = curItem == (itemCount - 1);
    ...
    public static class ReadResponseBuilder implements ExtensionObjectDefinitionIO.ExtensionObjectDefinitionBuilder {
        ...
        public ReadResponseBuilder(ExtensionObjectDefinition responseHeader, int noOfResults, DataValue[] results, int noOfDiagnosticInfos, DiagnosticInfo[] diagnosticInfos) {
        ...
        public ReadResponse build() {
            return new ReadResponse(responseHeader, noOfResults, results, noOfDiagnosticInfos, diagnosticInfos);
        }
    }
