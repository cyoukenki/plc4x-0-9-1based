/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
package org.apache.plc4x.java.opcua.readwrite.io;

import static org.apache.plc4x.java.spi.generation.StaticHelper.*;

import org.apache.plc4x.java.opcua.readwrite.*;
import org.apache.plc4x.java.opcua.readwrite.io.*;
import org.apache.plc4x.java.opcua.readwrite.types.*;
import org.apache.plc4x.java.api.exceptions.PlcRuntimeException;
import org.apache.plc4x.java.spi.generation.*;
import org.apache.plc4x.java.api.value.PlcValue;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.math.BigInteger;
import java.time.*;
import java.util.LinkedList;
import java.util.List;
import java.util.function.Supplier;

// Code generated by code-generation. DO NOT EDIT.

public class ServiceCounterDataTypeIO implements MessageIO<ServiceCounterDataType, ServiceCounterDataType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ServiceCounterDataTypeIO.class);

    @Override
    public ServiceCounterDataType parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (ServiceCounterDataType) new ExtensionObjectDefinitionIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, ServiceCounterDataType value, Object... args) throws ParseException {
        new ExtensionObjectDefinitionIO().serialize(writeBuffer, value, args);
    }

    public static ServiceCounterDataTypeBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("ServiceCounterDataType");
        int startPos = readBuffer.getPos();
        int curPos;


        // Simple Field (totalCount)
long totalCount = readBuffer.readUnsignedLong("totalCount", 32) ;

        // Simple Field (errorCount)
long errorCount = readBuffer.readUnsignedLong("errorCount", 32) ;
        readBuffer.closeContext("ServiceCounterDataType");
        // Create the instance
        return new ServiceCounterDataTypeBuilder(totalCount, errorCount);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, ServiceCounterDataType _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("ServiceCounterDataType");

        // Simple Field (totalCount)
        long totalCount = (long) _value.getTotalCount();
        writeBuffer.writeUnsignedLong("totalCount", 32, ((Number) (totalCount)).longValue());

        // Simple Field (errorCount)
        long errorCount = (long) _value.getErrorCount();
        writeBuffer.writeUnsignedLong("errorCount", 32, ((Number) (errorCount)).longValue());
        writeBuffer.popContext("ServiceCounterDataType");
    }

    public static class ServiceCounterDataTypeBuilder implements ExtensionObjectDefinitionIO.ExtensionObjectDefinitionBuilder {
        private final long totalCount;
        private final long errorCount;

        public ServiceCounterDataTypeBuilder(long totalCount, long errorCount) {
            this.totalCount = totalCount;
            this.errorCount = errorCount;
        }

        public ServiceCounterDataType build() {
            return new ServiceCounterDataType(totalCount, errorCount);
        }
    }

}
