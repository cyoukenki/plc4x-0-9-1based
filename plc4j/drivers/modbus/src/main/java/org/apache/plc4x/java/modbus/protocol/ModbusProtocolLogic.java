...
import org.apache.plc4x.java.modbus.util.ConvertUtil;
...
public class ModbusProtocolLogic extends Plc4xProtocolBase<ModbusTcpADU> implements HasConfiguration<ModbusConfiguration> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ModbusProtocolLogic.class);
    @Override
...
//                .onError((p, e) -> future.completeExceptionally(e))
    ...
    private ModbusPDU getWriteRequestPdu(PlcField field, PlcValue plcValue) {
        if(field instanceof ModbusFieldCoil) {
            ...
            ModbusFieldHoldingRegister holdingRegister = (ModbusFieldHoldingRegister) field;
            int lengthWords = holdingRegister.getLengthWords();
            if (holdingRegister.getDataType() == ModbusDataType.STRING) {
                int length = plcValue.getString().length();
                int addressLength = length + length % 2;
                int lengthBytes;
                if (holdingRegister.getLengthBytes() > addressLength) {
                    lengthBytes = addressLength;
                } else {
                    lengthBytes = holdingRegister.getLengthBytes() == 0 ? addressLength : holdingRegister.getLengthBytes() + holdingRegister.getLengthBytes() % 2;
                }
                ModbusPDUWriteMultipleHoldingRegistersRequest request = new ModbusPDUWriteMultipleHoldingRegistersRequest(holdingRegister.getAddress(), lengthBytes / 2, fromPlcValue(field, plcValue));
                return request;
            } else {
                ModbusPDUWriteMultipleHoldingRegistersRequest request = new ModbusPDUWriteMultipleHoldingRegistersRequest(holdingRegister.getAddress(), lengthWords, fromPlcValue(field, plcValue));
    ...
    private PlcValue toPlcValue(ModbusPDU request, ModbusPDU response, ModbusDataType dataType) throws ParseException {
        ...
        if (request instanceof ModbusPDUReadDiscreteInputsRequest) {
            ...
            if (dataType == ModbusDataType.STRING) {
                int numberOfValues = Math.max(Math.round(req.getQuantity() / fieldDataTypeSize), 1);
                if (EvaluationHelper.equals(dataType, ModbusDataType.STRING) && EvaluationHelper.equals(numberOfValues, 1)) { // STRING
                    Integer value = io.readUnsignedInt("", 16);
                    return new PlcSTRING(ConvertUtil.hexadecimalToString(Integer.toHexString(value)));
                } else if (EvaluationHelper.equals(dataType, ModbusDataType.STRING)) { // List
                    // Array field (value)
                    // Count array
                    ...
                    for (int curItem = 0; curItem < itemCount; curItem++) {
                        int i = io.readUnsignedInt("", 16);
                        if (i > 0) {
                            try {
                                String dataStr = ConvertUtil.hexadecimalToString(Integer.toHexString(i));
                                stringBuilder.append(dataStr);
                            } catch (Exception e) {
                                logger.error("hexadecimalToString error,please check data setting!");
                                break;
                            }
                        }
                    }
                    return new PlcSTRING(stringBuilder.toString());
                }
            }
            return DataItemIO.staticParse(io, dataType, Math.max(Math.round(req.getQuantity() / (fieldDataTypeSize / 2.0f)), 1));
    ...
    private byte[] fromPlcValue(PlcField field, PlcValue plcValue) {
        ...
        try {
            ...
            if(plcValue instanceof PlcList) {
                ...
                int lengthBytes = ((ModbusField) field).getLengthBytes() == 0 ? plcValue.toString().length() : ((ModbusField) field).getLengthBytes();
                PlcValue plcValueResult = null;
                String valueStr;
                if (plcValue.getString().length() >= lengthBytes) {
                    valueStr = plcValue.getString().substring(0, lengthBytes);
                } else {
                    valueStr = plcValue.getString();
                }
                if (valueStr.length() == 1) {
                    char high = valueStr.charAt(0);
                    int asciiCode = (int) high << 8 | (int) 0;
                    plcValueResult = new PlcUINT(asciiCode);
                } else if (valueStr.length() == 2) {
                    char high = valueStr.charAt(0);
                    char low = valueStr.charAt(1);
                    int asciiCode = (int) high << 8 | (int) low;
                    plcValueResult = new PlcUINT(asciiCode);
                } else {
                    plcValueResult = new PlcList();
                    for (int i = 0; i < valueStr.length(); i += 2) {
                        PlcUINT plcUINT = null;
                        if (i + 1 < valueStr.length()) {
                            char high = valueStr.charAt(i);
                            char low = valueStr.charAt(i + 1);
                            int asciiCode = (int) high << 8 | (int) low;
                            plcUINT = new PlcUINT(asciiCode);
                        } else if (i < valueStr.length()) { // 如果字符串长度为奇数，只处理最后一个字符
                            char high = valueStr.charAt(i);
                            int asciiCode = (int) high << 8 | (int) 0;
                            plcUINT = new PlcUINT(asciiCode);
                        }
                        ((PlcList) plcValueResult).add(plcUINT);
                    }
                }
                buffer = DataItemIO.staticSerialize(plcValueResult, ModbusDataType.UINT, valueStr.length() / 2 + valueStr.length() % 2, false);
                if (buffer != null) {
