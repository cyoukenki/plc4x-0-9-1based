...
public class WritebybitIO implements MessageIO<Writebybit, Writebybit> {

    private static final Logger LOGGER = LoggerFactory.getLogger(WritebybitIO.class);

    @Override
    public Writebybit parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (Writebybit) new WriteSubcommandTypeIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, Writebybit value, Object... args) throws ParseException {
        new WriteSubcommandTypeIO().serialize(writeBuffer, value, args);
    }

    public static WritebybitBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("Writebybit");
...
long devicepointno = readBuffer.readUnsignedLong("devicepointno", 32) ;        // Array field (data)
        ...
        // Count array
        if(devicepointno > Integer.MAX_VALUE) {
            throw new ParseException("Array count of " + (devicepointno) + " exceeds the maximum allowed count of " + Integer.MAX_VALUE);
        }
        byte[] data;
        {
            int itemCount = Math.max(0, (int) devicepointno);
...
data[curItem] = readBuffer.readSignedByte("", 8) ;            }
        ...
        readBuffer.closeContext("Writebybit");
        // Create the instance
        return new WritebybitBuilder(devicecode, headdeviceno, devicepointno, data);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, Writebybit _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("Writebybit");
        ...
        writeBuffer.popContext("Writebybit");
    }

    public static class WritebybitBuilder implements WriteSubcommandTypeIO.WriteSubcommandTypeBuilder {
        private final Devicecode devicecode;
        private final BigInteger headdeviceno;
        private final long devicepointno;
        private final byte[] data;

        public WritebybitBuilder(Devicecode devicecode, BigInteger headdeviceno, long devicepointno, byte[] data) {
        ...
        public Writebybit build() {
            return new Writebybit(devicecode, headdeviceno, devicepointno, data);
        }
    }
