...
public class ReadbybitIO implements MessageIO<Readbybit, Readbybit> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ReadbybitIO.class);

    @Override
    public Readbybit parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (Readbybit) new ReadSubcommandTypeIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, Readbybit value, Object... args) throws ParseException {
        new ReadSubcommandTypeIO().serialize(writeBuffer, value, args);
    }

    public static ReadbybitBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("Readbybit");
...
long devicepointno = readBuffer.readUnsignedLong("devicepointno", 32) ;
        readBuffer.closeContext("Readbybit");
        // Create the instance
        return new ReadbybitBuilder(devicecode, headdeviceno, devicepointno);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, Readbybit _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("Readbybit");
        ...
        writeBuffer.popContext("Readbybit");
    }

    public static class ReadbybitBuilder implements ReadSubcommandTypeIO.ReadSubcommandTypeBuilder {
        private final Devicecode devicecode;
        private final BigInteger headdeviceno;
        private final long devicepointno;

        public ReadbybitBuilder(Devicecode devicecode, BigInteger headdeviceno, long devicepointno) {
            this.devicecode = devicecode;
            this.headdeviceno = headdeviceno;
            this.devicepointno = devicepointno;
        }

        public Readbybit build() {
            return new Readbybit(devicecode, headdeviceno, devicepointno);
        }
    }
