...
public class AlarmMessageObjectAckTypeIO implements MessageIO<AlarmMessageObjectAckType, AlarmMessageObjectAckType> {
    ...
    @Override
    public AlarmMessageObjectAckType parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return AlarmMessageObjectAckTypeIO.staticParse(readBuffer);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, AlarmMessageObjectAckType value, Object... args) throws ParseException {
        AlarmMessageObjectAckTypeIO.staticSerialize(writeBuffer, value);
    }
    ...
    public static AlarmMessageObjectAckType staticParse(ReadBuffer readBuffer) throws ParseException {
        ...
        if(variableSpec != AlarmMessageObjectAckType.VARIABLESPEC) {
            throw new ParseException("Expected constant value " + AlarmMessageObjectAckType.VARIABLESPEC + " but got " + variableSpec);
        }

        // Const Field (length)
        short length = readBuffer.readUnsignedShort("length", 8);
        if(length != AlarmMessageObjectAckType.LENGTH) {
            throw new ParseException("Expected constant value " + AlarmMessageObjectAckType.LENGTH + " but got " + length);
        }
...
State ackStateComing = StateIO.staticParse(readBuffer ) ;        readBuffer.closeContext("ackStateComing");
    ...
    public static void staticSerialize(WriteBuffer writeBuffer, AlarmMessageObjectAckType _value) throws ParseException {
