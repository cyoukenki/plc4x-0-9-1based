...
public class AlarmMessageAckPushTypeIO implements MessageIO<AlarmMessageAckPushType, AlarmMessageAckPushType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AlarmMessageAckPushTypeIO.class);

    @Override
    public AlarmMessageAckPushType parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return AlarmMessageAckPushTypeIO.staticParse(readBuffer);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, AlarmMessageAckPushType value, Object... args) throws ParseException {
        AlarmMessageAckPushTypeIO.staticSerialize(writeBuffer, value);
    }
...
short numberOfObjects = readBuffer.readUnsignedShort("numberOfObjects", 8) ;        // Array field (messageObjects)
        ...
        AlarmMessageAckObjectPushType[] messageObjects;
        {
            int itemCount = Math.max(0, (int) numberOfObjects);
            messageObjects = new AlarmMessageAckObjectPushType[itemCount];
            for(int curItem = 0; curItem < itemCount; curItem++) {
                boolean lastItem = curItem == (itemCount - 1);
messageObjects[curItem] = AlarmMessageAckObjectPushTypeIO.staticParse(readBuffer ) ;            }
        }
        ...
        // Create the instance
        return new AlarmMessageAckPushType(TimeStamp, functionId, numberOfObjects, messageObjects);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, AlarmMessageAckPushType _value) throws ParseException {
        ...
        if(_value.getMessageObjects() != null) {
            ...
            int curItem = 0;
            for(AlarmMessageAckObjectPushType element : _value.getMessageObjects()) {
                boolean lastItem = curItem == (itemCount - 1);
                AlarmMessageAckObjectPushTypeIO.staticSerialize(writeBuffer, element);
                curItem++;
            }
