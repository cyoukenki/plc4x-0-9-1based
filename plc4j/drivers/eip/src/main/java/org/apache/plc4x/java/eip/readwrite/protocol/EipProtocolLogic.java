    ...
    public CompletableFuture<PlcReadResponse> read(PlcReadRequest readRequest) {
        ...
        try {
            ...
            for (int i = 0; i < singleRespOverlengthParams.size(); i++) {
                ...
                Map<String, ResponseItem<PlcValue>> mergeSegmentResponse = mergeSegmentPlcReadResponse(fieldName, field, dataByteLength);
            ...
            for (int i = 0; i < noSingleRespOverlengthParams.size(); i++) {
                ...
                if (requests.size() > 0) {
                    Map<String, ResponseItem<PlcValue>> responseMap = readPlcResponse(eipRequestEntities, request, requests);
...
//        splitRequestParams(eipFiledMap, fields);//计算请求数据长度，报文长度大于502 拆分
//        Map<String, ResponseItem<PlcValue>> responseItemMap = new LinkedHashMap<>();
//        //TODO  请求拆分后  再根据返回包长度进行二次拆分  组合不超长请求/单个超长请求
...
//            int dataByteLength = calculateSingleReadRespDataLength(plcField);//计算数据长度  用于判断单个tag报文返回结果是否超长
//            if (dataByteLength < dataPackageByteLength) {
//                //单个数据不超长情况
//                Map<String, ResponseItem<PlcValue>> singgleResponseItemMap = readSinglePlcResponse(fieldName, plcField);
//                responseItemMap.put(fieldName, singgleResponseItemMap.get(fieldName));
//            } else {
//                //单个数据超长 分段请求 合并结果
//                Map<String, ResponseItem<PlcValue>> mergeSegmentResponse = mergeSegmentPlcReadResponse(fieldName, plcField, dataByteLength);
//                responseItemMap.put(fieldName, mergeSegmentResponse.get(fieldName));
//            }
//        }
//        CompletableFuture<PlcReadResponse> completableFuture = new CompletableFuture<>();
...
//        return toPlcReadResponse(readRequest, readInternal(requests));
    ...
    private Map<String, ResponseItem<PlcValue>> readPlcResponse(List<String> eipRequestEntities, PlcReadRequest readRequest, List<CipReadRequest> requests) {
        CompletableFuture<Map<String, ResponseItem<PlcValue>>> mapCompletableFuture = getPlcReadResponse(eipRequestEntities, readRequest, readInternal(requests));
    ...
    private CompletableFuture<Map<String, ResponseItem<PlcValue>>> getPlcReadResponse(List<String> eipRequestEntities, PlcReadRequest readRequest, CompletableFuture<CipService> response) {
        return response
            .thenApply(p -> {
                ...
                if (p instanceof CipReadResponse) {
                    ...
                    if (status == 0) {
                        ...
                        values.put(eipRequestEntities.get(0), new ResponseItem<>(PlcResponseCode.INTERNAL_ERROR, plcValue));
                    }
    ...
    private Map<String, ResponseItem<PlcValue>> mergeSegmentPlcReadResponse(String fieldName, EipField plcField, int dataByteLength) {
        ...
        for (int i = 0; i < segementNum; i++) {
            int offsetStartByte1 = Integer.parseInt(String.format("%04X", offsetStart + (i * offset)).substring(0, 2), 16);
            int offsetStartByte2 = Integer.parseInt(String.format("%04X", offsetStart + (i * offset)).substring(2, 4), 16);
            ...
            byte[] segementBytes = new byte[]{(byte) 0x80, (byte) 0x03, (byte) offsetStartByte2, (byte) offsetStartByte1, (byte) 0x00, (byte) 0x00, (byte) offsetByte2, (byte) offsetByte1};
            CipReadRequest req;
            if (isArray) {
                req = new CipReadRequest(getSegmentRequestSize(tagIsolated), toAnsiForArray(tagIsolated, segementBytes), elements);
            } else {
                req = new CipReadRequest(getSegmentRequestSize(tagIsolated), toAnsi(tagIsolated, segementBytes), elements);
            }
...
//            elements = leftPackegeByte/8;
            int offsetStart1 = Integer.parseInt(String.format("%04X", offsetStart + segementNum * dataPackageByteLength).substring(0, 2), 16);
            int offsetStart2 = Integer.parseInt(String.format("%04X", offsetStart + segementNum * dataPackageByteLength).substring(2, 4), 16);
            ...
            byte[] segementBytes = new byte[]{(byte) 0x80, (byte) 0x03, (byte) offsetStart2, (byte) offsetStart1, (byte) 0x00, (byte) 0x00, (byte) offset2, (byte) offset1};
            CipReadRequest req;
            if (isArray) {
                req = new CipReadRequest(getSegmentRequestSize(tagIsolated), toAnsiForArray(tagIsolated, segementBytes), elements);
            } else {
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            CompletableFuture<Map<String, ResponseItem<PlcValue>>> mapCompletableFuture = toPlcReadSegmentResponse(fieldName, plcField, initialOffset, readSingleInternal(requests.get(i)));
            try {
                Map<String, ResponseItem<PlcValue>> responseSingleItemMap = mapCompletableFuture.get(REQUEST_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
    ...
    private CompletableFuture<Map<String, ResponseItem<PlcValue>>> toPlcReadSegmentResponse(String fieldName, EipField eipField, int initialOffset, CompletableFuture<CipService> response) {
        return response
            .thenApply(p -> {
                Map<String, ResponseItem<PlcValue>> values = new HashMap<>();
                // only 1 field
    ...
    private CompletableFuture<CipService> readInternal(List<CipReadRequest> request) {
        ...
        if (request.size() > 1) {
            ...
            CipExchange exchange = new CipExchange(new CipUnconnectedRequest(request.get(0),
                (byte) configuration.getBackplane(), (byte) configuration.getSlot()));
    ...
    private CompletableFuture<CipService> readSingleInternal(CipReadRequest request) {
        ...
        this.tm = new RequestTransactionManager(1);
        RequestTransactionManager.RequestTransaction transaction = tm.startRequest();
        CipExchange exchange = new CipExchange(new CipUnconnectedRequest(request,
            (byte) configuration.getBackplane(), (byte) configuration.getSlot()));
        CipRRData pkt = new CipRRData(sessionHandle, 0L, emptySenderContext, 0L, exchange);
        transaction.submit(() -> context.sendRequest(pkt)
            .expectResponse(EipPacket.class, REQUEST_TIMEOUT)
                ...
                switch (type) {
                    ...
                    case STRING:
                        ...
                        list.add(new PlcSTRING(StandardCharsets
                            .UTF_8.decode(data.nioBuffer(2, aShort)).toString()));
                        index += aShort + 2;
                        break;
                    default:
                        return null;
    ...
    public CompletableFuture<PlcWriteResponse> write(PlcWriteRequest writeRequest) {
        ...
        for (int i = 0; i < singleRespOverlengthParams.size(); i++) {
            ...
            Map<String, PlcResponseCode> plcResponseCodeMap = mergeSegmentPlcWriteResponse(fieldName, field, data, allowPackageLength);
...
//            //计算请求数据的最大允许长度（502减公共部分）
//            int allowPackageLength = calculateWriteReqAllowPackageLength(fieldName);
//            if (data.length > allowPackageLength) {
//                //拆分请求
//                Map<String, PlcResponseCode> plcResponseCodeMap = mergeSegmentPlcWriteResponse(fieldName, field, data, allowPackageLength);
//                responseItemMap.put(fieldName, plcResponseCodeMap.get(fieldName));
//            } else {
//                Map<String, PlcResponseCode> singgleResponseItemMap = singlePlcWriteResponse(writeRequest, fieldName);
//                responseItemMap.put(fieldName, singgleResponseItemMap.get(fieldName));
//            }
//        }
//        future.complete(new DefaultPlcWriteResponse(writeRequest, responseItemMap));
...
//                new CipExchange(
//                    new CipUnconnectedRequest(items.get(0), (byte) configuration.getBackplane(),
//                        (byte) configuration.getSlot())));
...
//        return future;
    ...
    private Map<String, PlcResponseCode> mergeMutiplePlcWriteResponse(List<String> params, PlcWriteRequest writeRequest, List<CipWriteRequest> items) {
    ...
    private Map<String, PlcResponseCode> mergeSegmentPlcWriteResponse(String fieldName, EipField plcField, byte[] data, int allowPackageLength) {
        Map<String, PlcResponseCode> plcResponseCodeMap = new HashMap<>();
        RequestTransactionManager tm = new RequestTransactionManager(1);
        RequestTransactionManager.RequestTransaction transaction = tm.startRequest();
        int arrayIndex = 0;//用于记录数组请求的起始索引
        ...
        for (int i = 0; i < segementNum; i++) {
            int offsetStartByte1 = Integer.parseInt(String.format("%04X", offsetStart + (i * offset)).substring(0, 2), 16);
            int offsetStartByte2 = Integer.parseInt(String.format("%04X", offsetStart + (i * offset)).substring(2, 4), 16);
            int offsetByte1 = Integer.parseInt(String.format("%04X", offset + (i * allowPackageLength)).substring(0, 2), 16);
            int offsetByte2 = Integer.parseInt(String.format("%04X", offset + (i * allowPackageLength)).substring(2, 4), 16);
            byte[] segementBytes = new byte[]{(byte) 0x80, (byte) 0x03, (byte) offsetStartByte2, (byte) offsetStartByte1, (byte) 0x00, (byte) 0x00, (byte) offsetByte2, (byte) offsetByte1};
            CipWriteRequest writeReq = new CipWriteRequest(getSegmentRequestSize(tagIsolated), toAnsiForArray(tagIsolated, segementBytes), plcField.getType(), elements,
                data);
            requests.add(writeReq);
        }
        if (leftPackegeByte > 0) {
            int offsetStartByte1 = Integer.parseInt(String.format("%04X", offsetStart + (segementNum * offset)).substring(0, 2), 16);
            int offsetStartByte2 = Integer.parseInt(String.format("%04X", offsetStart + (segementNum * offset)).substring(2, 4), 16);
    ...
    private int calculateWriteReqAllowPackageLength(String fieldName) {
        ...
        allowDataBytes = EIPConfiguration.UCMM_PACKAGE_LENGTH - (prefixBytelength + dataLengthBytes + paramDataTypeBytes + paramLengthBytes + paramBytes + dataTypeBytes + reversedBytes + endBytes);
    ...
    private Map<String, PlcResponseCode> decodeWriteResponse(CipService p, PlcWriteRequest writeRequest, List<String> params) {
