...
public class FinsUdpProtocolLogic extends Plc4xProtocolBase<FinsUdpPacket>
        implements HasConfiguration<FinsConfiguration> {

    private static final Logger logger = LoggerFactory.getLogger(FinsUdpProtocolLogic.class);
    public static final Duration REQUEST_TIMEOUT = Duration.ofMillis(20000);

    // private static final short[] emptySenderContext = new short[]{(short) 0x00,
    ...
    @Override
    public void onConnect(ConversationContext<FinsUdpPacket> context) {
        context.fireConnected();
        logger.info("Fins udp is connected ...");
      
    }
    @Override
    public CompletableFuture<PlcReadResponse> read(PlcReadRequest readRequest) {
        // errorCode ICF RSV VCT DNA DA1 DA2 SNA SA1 SA2 SID dataType wordStart
        // bitsStart lenght
        // 需动态传入 FinsDataTypeCode,wordStart,lenght
        // ReadBitsReq readBitsReq = new
        // ReadBitsReq(0L,(short)0x80,(short)0x00,(short)0x02,(short)0x00,remoteSA1,(short)0x00,(short)0x00,hostSA1,(short)0x00,(short)0x00,
        // FinsDataTypeCode.D,100,(short)0x00,(short)0x00);
        ...
        if (request.getFieldNames().size() == 1) {
            ...
            // UdpReadBitsReq(short DA1, short SA1, short SID, FinsDataTypeCode dataType,
            // int wordStart, short bitsStart, int lenght) {

            UdpReadBitsReq readBitsReq = new UdpReadBitsReq((short)0x80,(short) configuration.getDa(),
                    (short) configuration.getSa(), (short) new Random().nextInt(), areaType, wordStartInt,
                    (short) bitsStart, (short) quantity);
            RequestTransactionManager.RequestTransaction transaction = tm.startRequest();
            transaction.submit(() -> context.sendRequest(readBitsReq)
                    .expectResponse(FinsUdpPacket.class, REQUEST_TIMEOUT)
                    .onTimeout((e) -> {
                    ...
                    .handle(p -> {
                        ...
                        // logger.info("res:"+p.toString());
                        if (p instanceof UdpReadBitsRes) {
                            UdpReadBitsRes readBitsRes = (UdpReadBitsRes) p;
                            byte[] data = readBitsRes.getData();
                            // logger.info("data length:"+data.length+";"+readBitsRes.getEndCode()+";"+readBitsRes.toString());
    ...
    public static Long bcdDataTransfer(PlcValue plcValue, FinsDataType dataType) {
        ...
        switch (dataType) {
            ...
            case UINT_BCD:
                ...
                binaryString = uintBinaryString.length() > 16
                        ? uintBinaryString.substring(uintBinaryString.length() - 16)
                        : uintBinaryString;
                break;
            case ULINT_BCD:
    ...
    public CompletableFuture<PlcWriteResponse> write(PlcWriteRequest writeRequest) {
        ...
        List<UdpWriteBitsReq> items = new ArrayList<>(writeRequest.getNumberOfFields());
        for (String fieldName : request.getFieldNames()) {
            ...
            UdpWriteBitsReq commandDataStruct = new UdpWriteBitsReq((short)0x80,(short) configuration.getDa(),
                    (short) configuration.getSa(), (short) new Random().nextInt(),
                    areaType, wordStartInt,
                    (short) bitsStart,
                    (short) quantity, data);
            items.add(commandDataStruct);
        }
        RequestTransactionManager.RequestTransaction transaction = tm.startRequest();
        if (items.size() == 1) {
            transaction.submit(() -> context.sendRequest(items.get(0))
                    ...
                    .check(p -> p instanceof FinsUdpPacket).unwrap(p -> (FinsUdpPacket) p)
                    .handle(p -> {
                        ...
                        if (p instanceof UdpWriteBitsRes) {
                            UdpWriteBitsRes writeBitsRes = (UdpWriteBitsRes) p;
    ...
    private PlcValue parsePlcValue(FinsField field, ByteBuf data, FinsDataType dataType, int quantity) {
        int index = 0;
        int dataSize = 0;
        if (dataType == FinsDataType.BOOL || dataType == FinsDataType.SINT || dataType == FinsDataType.USINT
                || dataType == FinsDataType.BYTE) {
        ...
        if (dataSize == 1) {
            ...
            switch (dataType) {
                ...
                case UDINT_BCD:
                    try {
                        plcValue = new PlcLINT(
                                bcdDataTransfer(new PlcUDINT(data.getUnsignedIntLE(index)), FinsDataType.UDINT_BCD));
                    } catch (Exception e) {
                ...
                case UINT_BCD:
                    try {
                        plcValue = new PlcUINT(
                                bcdDataTransfer(new PlcUINT(data.getUnsignedShort(index)), FinsDataType.UINT_BCD));
                    } catch (Exception e) {
                ...
                case ULINT:
                    try {
                        ...
                        logger.error("value is out of range 0 - 18446744073709551615 for a PlcULINT Value invalid {}",
                                e.getMessage());
                    }
                    return plcValue;
                case ULINT_BCD:
                    try {
                        plcValue = new PlcLINT(
                                bcdDataTransfer(new PlcLINT(data.getLongLE(index)), FinsDataType.ULINT_BCD));
                    } catch (Exception e) {
            ...
            for (int i = 0; i < dataSize; i++) {
                switch (dataType) {
                    ...
                    case UDINT_BCD:
                        try {
                            list.add(new PlcLINT(bcdDataTransfer(new PlcUDINT(data.getUnsignedIntLE(index)),
                                    FinsDataType.UDINT_BCD)));
                        } catch (Exception e) {
                            logger.error("transfer UDINT_BCD data error", e.getMessage());
                        }
                        index += dataType.getDataTypeSize();
                    ...
                    case ULINT:
                        try {
                            ...
                            logger.error(
                                    "value is out of range 0 - 18446744073709551615 for a PlcULINT Value invalid {}",
                                    e.getMessage());
                        }
                        index += dataType.getDataTypeSize();
                        break;
    ...
    private byte[] encodeValue(PlcValue value, FinsDataType type, int elements) {
        ...
        if (type == STRING) {
            ...
            buffer = ByteBuffer.allocate(type.getDataTypeSize() * 2 * asciiValues.length)
        ...
        if (elements > 1) {
            ...
            for (int i = 0; i < elements; i++) {
                ...
                switch (type) {
                    ...
                    case BOOL:
                        ...
                        break;
                    case UINT_BCD:
                        buffer.putShort((short) Integer.parseInt(
                                String.valueOf(parseBcdData(plcValue.getShort(), FinsDataType.UINT_BCD).getShort()),
                                16));
                        break;
                    case UDINT_BCD:
                        buffer.putInt((int) Long.parseLong(
                                String.valueOf(parseBcdData(plcValue.getInt(), FinsDataType.UDINT_BCD).getLong()), 16));
                        break;
                    case ULINT_BCD:
                        buffer.putLong(Long.parseUnsignedLong(
                                String.valueOf(parseBcdData(plcValue.getLong(), FinsDataType.ULINT_BCD).getLong()),
                                16));
                        break;
                    default:
                        break;
            ...
            switch (type) {
                ...
                case UINT_BCD:
                    buffer.putShort((short) Integer.parseInt(
                            String.valueOf(parseBcdData(value.getShort(), FinsDataType.UINT_BCD).getShort()), 16));
                    break;
                case UDINT_BCD:
                    buffer.putInt((int) Long.parseLong(
                            String.valueOf(parseBcdData(value.getInt(), FinsDataType.UDINT_BCD).getInt()), 16));
                    break;
                case ULINT_BCD:
                    buffer.putLong(Long.parseUnsignedLong(
                            String.valueOf(parseBcdData(value.getLong(), FinsDataType.ULINT_BCD).getLong()), 16));
                    break;
                default:
                    break;
            }
    ...
    @Override
    public void close(ConversationContext<FinsUdpPacket> context) {
        context.fireDisconnected();
        logger.debug("close------------------------------");
    }

    @Override
    public void onDisconnect(ConversationContext<FinsUdpPacket> context) {
