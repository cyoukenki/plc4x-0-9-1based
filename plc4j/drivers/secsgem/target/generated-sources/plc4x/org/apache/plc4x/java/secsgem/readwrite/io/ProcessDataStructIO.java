...
public class ProcessDataStructIO implements MessageIO<ProcessDataStruct, ProcessDataStruct> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ProcessDataStructIO.class);

    @Override
    public ProcessDataStruct parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return ProcessDataStructIO.staticParse(readBuffer);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, ProcessDataStruct value, Object... args) throws ParseException {
        ProcessDataStructIO.staticSerialize(writeBuffer, value);
    }

    public static ProcessDataStruct staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("ProcessDataStruct");
        int startPos = readBuffer.getPos();
        int curPos;

        // Const Field (L2)
        int L2 = readBuffer.readUnsignedInt("L2", 16);
        if(L2 != ProcessDataStruct.L2) {
            throw new ParseException("Expected constant value " + ProcessDataStruct.L2 + " but got " + L2);
        }

        readBuffer.pullContext("CCODE");

        // Simple Field (CCODE)
DataStruct CCODE = DataStructIO.staticParse(readBuffer ) ;        readBuffer.closeContext("CCODE");
...
values[curItem] = DataStructIO.staticParse(readBuffer ) ;            }
        ...
        readBuffer.closeContext("ProcessDataStruct");
        // Create the instance
        return new ProcessDataStruct(CCODE, symbolTypeDS, values);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, ProcessDataStruct _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("ProcessDataStruct");

        // Const Field (L2)
        writeBuffer.writeUnsignedInt("L2", 16, ((Number) 0x0102).intValue());

        // Simple Field (CCODE)
        DataStruct CCODE = (DataStruct) _value.getCCODE();
        writeBuffer.pushContext("CCODE");
        DataStructIO.staticSerialize(writeBuffer, CCODE);
        writeBuffer.popContext("CCODE");
        ...
        writeBuffer.popContext("ProcessDataStruct");
    }
