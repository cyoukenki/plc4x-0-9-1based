...
public class S2F35RequestIO implements MessageIO<S2F35Request, S2F35Request> {

    private static final Logger LOGGER = LoggerFactory.getLogger(S2F35RequestIO.class);

    @Override
    public S2F35Request parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (S2F35Request) new SecsPacketIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, S2F35Request value, Object... args) throws ParseException {
        new SecsPacketIO().serialize(writeBuffer, value, args);
    }

    public static S2F35RequestBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("S2F35Request");
        int startPos = readBuffer.getPos();
        int curPos;

        // Const Field (list2)
        int list2 = readBuffer.readUnsignedInt("list2", 16);
        if(list2 != S2F35Request.LIST2) {
            throw new ParseException("Expected constant value " + S2F35Request.LIST2 + " but got " + list2);
        }
...
DataStruct2 symbolTypeDS = DataStruct2IO.staticParse(readBuffer ) ;        readBuffer.closeContext("symbolTypeDS");
        ...
        CEIDListDataStruct[] values;
        {
            int itemCount = Math.max(0, (int) symbolTypeDS.getIdentifier());
            values = new CEIDListDataStruct[itemCount];
            for(int curItem = 0; curItem < itemCount; curItem++) {
                boolean lastItem = curItem == (itemCount - 1);
values[curItem] = CEIDListDataStructIO.staticParse(readBuffer ) ;            }
        }
            readBuffer.closeContext("values", WithReaderWriterArgs.WithRenderAsList(true));

        readBuffer.closeContext("S2F35Request");
        // Create the instance
        return new S2F35RequestBuilder(dataId, symbolTypeDS, values);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, S2F35Request _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("S2F35Request");
        ...
        if(_value.getValues() != null) {
            ...
            int curItem = 0;
            for(CEIDListDataStruct element : _value.getValues()) {
                boolean lastItem = curItem == (itemCount - 1);
                CEIDListDataStructIO.staticSerialize(writeBuffer, element);
                curItem++;
            }
            writeBuffer.popContext("values", WithReaderWriterArgs.WithRenderAsList(true));
        }
        writeBuffer.popContext("S2F35Request");
    }

    public static class S2F35RequestBuilder implements SecsPacketIO.SecsPacketBuilder {
        private final DataStruct dataId;
        private final DataStruct2 symbolTypeDS;
        private final CEIDListDataStruct[] values;

        public S2F35RequestBuilder(DataStruct dataId, DataStruct2 symbolTypeDS, CEIDListDataStruct[] values) {
            this.dataId = dataId;
            this.symbolTypeDS = symbolTypeDS;
            this.values = values;
        }

        public S2F35Request build(int deviceID, short PType, short Stype, long systemBytes) {
            return new S2F35Request(deviceID, PType, Stype, systemBytes, dataId, symbolTypeDS, values);
        }
    }
