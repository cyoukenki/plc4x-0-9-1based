...
public class S1F3RequestIO implements MessageIO<S1F3Request, S1F3Request> {

    private static final Logger LOGGER = LoggerFactory.getLogger(S1F3RequestIO.class);

    @Override
    public S1F3Request parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (S1F3Request) new SecsPacketIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, S1F3Request value, Object... args) throws ParseException {
        new SecsPacketIO().serialize(writeBuffer, value, args);
    }

    public static S1F3RequestBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("S1F3Request");
...
values[curItem] = DataStructIO.staticParse(readBuffer ) ;            }
        ...
        readBuffer.closeContext("S1F3Request");
        // Create the instance
        return new S1F3RequestBuilder(symbolTypeDS, values);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, S1F3Request _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("S1F3Request");
        ...
        writeBuffer.popContext("S1F3Request");
    }

    public static class S1F3RequestBuilder implements SecsPacketIO.SecsPacketBuilder {
        private final DataStruct2 symbolTypeDS;
        private final DataStruct[] values;

        public S1F3RequestBuilder(DataStruct2 symbolTypeDS, DataStruct[] values) {
            this.symbolTypeDS = symbolTypeDS;
            this.values = values;
        }

        public S1F3Request build(int deviceID, short PType, short Stype, long systemBytes) {
            return new S1F3Request(deviceID, PType, Stype, systemBytes, symbolTypeDS, values);
        }
    }
