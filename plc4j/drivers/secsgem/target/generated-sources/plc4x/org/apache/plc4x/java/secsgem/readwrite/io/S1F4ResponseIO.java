...
public class S1F4ResponseIO implements MessageIO<S1F4Response, S1F4Response> {

    private static final Logger LOGGER = LoggerFactory.getLogger(S1F4ResponseIO.class);

    @Override
    public S1F4Response parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (S1F4Response) new SecsPacketIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, S1F4Response value, Object... args) throws ParseException {
        new SecsPacketIO().serialize(writeBuffer, value, args);
    }

    public static S1F4ResponseBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("S1F4Response");
...
values[curItem] = DataStructIO.staticParse(readBuffer ) ;            }
        ...
        readBuffer.closeContext("S1F4Response");
        // Create the instance
        return new S1F4ResponseBuilder(symbolTypeDS, values);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, S1F4Response _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("S1F4Response");
        ...
        writeBuffer.popContext("S1F4Response");
    }

    public static class S1F4ResponseBuilder implements SecsPacketIO.SecsPacketBuilder {
        private final DataStruct2 symbolTypeDS;
        private final DataStruct[] values;

        public S1F4ResponseBuilder(DataStruct2 symbolTypeDS, DataStruct[] values) {
            this.symbolTypeDS = symbolTypeDS;
            this.values = values;
        }

        public S1F4Response build(int deviceID, short PType, short Stype, long systemBytes) {
            return new S1F4Response(deviceID, PType, Stype, systemBytes, symbolTypeDS, values);
        }
    }
