...
public class S7F6ResponseIO implements MessageIO<S7F6Response, S7F6Response> {

    private static final Logger LOGGER = LoggerFactory.getLogger(S7F6ResponseIO.class);

    @Override
    public S7F6Response parse(ReadBuffer readBuffer, Object... args) throws ParseException {
        return (S7F6Response) new SecsPacketIO().parse(readBuffer, args);
    }

    @Override
    public void serialize(WriteBuffer writeBuffer, S7F6Response value, Object... args) throws ParseException {
        new SecsPacketIO().serialize(writeBuffer, value, args);
    }

    public static S7F6ResponseBuilder staticParse(ReadBuffer readBuffer) throws ParseException {
        readBuffer.pullContext("S7F6Response");
        ...
        if(L != S7F6Response.L) {
            throw new ParseException("Expected constant value " + S7F6Response.L + " but got " + L);
        }

        // Const Field (Number)
        short Number = readBuffer.readUnsignedShort("Number", 8);
        if(Number != S7F6Response.NUMBER) {
            throw new ParseException("Expected constant value " + S7F6Response.NUMBER + " but got " + Number);
        }
...
DataStruct3 symbolTypeDS = DataStruct3IO.staticParse(readBuffer ) ;        readBuffer.closeContext("symbolTypeDS");
        ...
        readBuffer.closeContext("S7F6Response");
        // Create the instance
        return new S7F6ResponseBuilder(PPID, symbolTypeDS, data);
    }

    public static void staticSerialize(WriteBuffer writeBuffer, S7F6Response _value) throws ParseException {
        int startPos = writeBuffer.getPos();
        writeBuffer.pushContext("S7F6Response");
        ...
        writeBuffer.popContext("S7F6Response");
    }

    public static class S7F6ResponseBuilder implements SecsPacketIO.SecsPacketBuilder {
        private final DataStruct PPID;
        private final DataStruct3 symbolTypeDS;
        private final byte[] data;

        public S7F6ResponseBuilder(DataStruct PPID, DataStruct3 symbolTypeDS, byte[] data) {
            this.PPID = PPID;
            this.symbolTypeDS = symbolTypeDS;
            this.data = data;
        }

        public S7F6Response build(int deviceID, short PType, short Stype, long systemBytes) {
            return new S7F6Response(deviceID, PType, Stype, systemBytes, PPID, symbolTypeDS, data);
        }
    }
